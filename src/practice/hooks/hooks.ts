...
export function useCrudTable() {
    ...
    const handleAdd = async (type: any, id: any) => {
        const { dialog, editorRef, subclassification } = this.dialog()
        dialog.visible = true;
        dialog.isView = false;
        if (type === 'add') {
            dialog.title = '新增';
    ...
    const handleSet = (row: any) => {
        console.log(row);
        const dialog = useScriptDialog()
        
        dialog.scrip.visible = true;
        dialog.scrip.form = { ...row };
        console.log(dialog.scrip);
    };
